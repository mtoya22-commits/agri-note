...
function spotsOf(p){ return Array.isArray(p.spots) && p.spots.length ? p.spots : (p.slots||[]).map(slotSpot); }
...
function rotAlong(sp, crop){
  const h = ((crop && crop.spacingCm) || DEFAULT_SPACING) / 2;
  let r = sp.kind==="plant" ? { from:sp.center-h, to:sp.center+h } : { from:sp.from, to:sp.to };
  if(sp.precision!=="slot") r = { from:r.from-ROT_BUFFER, to:r.to+ROT_BUFFER };
  return r;
}
...
function rowFor(table, o, deleted){
  ...
  if(table==="plantings"){
    ...
    return Object.assign({ id:o.id, bedId:o.bedId, slots:(o.slots||[]).join(","), cropId:o.cropId, cropName:c?c.name:"",
      date:o.date, count:o.count||0, status:o.status, endDate:o.endDate||"", memo:o.memo||"", spots:spotsText(o) }, meta);
  }
  if(table==="logs"){
...
function rotationCheck(slots, cropId, dateStr, ignoreId){
  ...
  slots.forEach(s=>{
    let worst = null;
    const bedId = slotBed(s), mine = slotSpot(s);
...
function checkPlanting(slots, cropId, dateStr, ignoreId){
  ...
  slots.forEach(s=> plantingsIn(s, p=>p.id!==ignoreId).forEach(p=>{
    const iv = intervalIn(p, s); if(overlaps(cand, iv)) clash.push({s, p, iv});
  }));
  if(clash.length){
...
function openPlanting(pid){
  ...
  let h = `<h3>${esc(c.name)}</h3>
    <div class="sheet-sub">${esc(bed.name)} 区画${slotNums(p)}（${esc(shareLabel(slotsTouched(p).length))}）・${p.count}株</div>
