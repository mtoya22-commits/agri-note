...
/* =============================================================
   ...
   作付けは畝の中の「位置」を持つ（spots）。保存するのは事実としての位置だけ：
     株      { kind:"plant", center }            … 植えた中心（左端からのcm）
     まとまり { kind:"group", from, to, n }      … 株がまとまっている範囲と株数
...
function rotAlong(sp, crop){
  const sc = spacingOf(crop), h = sc ? sc.preferred/2 : 0;          // 株間が不明なら株の中心＋根の余白だけ
...
function spotsText(p){
  ...
  return p.spots.map(sp=>{
    let t = sp.kind==="plant" ? `株${sp.center}` : sp.kind==="group" ? `群${sp.from}-${sp.to}×${sp.n||1}` : `帯${sp.from}-${sp.to}`;
...
function renderField(){
  ...
  BEDS.forEach(bed=>{
    ...
    h += `<div class="card bedcard" data-b="${bed.id}">
      ...
      ${bedBarHtml(bed.id)}
      <div class="barfoot"><span class="hint">空いている所をタップして株を置く</span><button class="btn small" data-act="place-open" data-b="${bed.id}">＋ 株を置く</button></div>
...
/* =============================================================
   シート（下から出る画面）
   ============================================================= */
let sheetCtx = null, ps = null, pe = null, pp = null;
function showSheet(h){ $("#sheet").innerHTML = h; $("#backdrop").hidden = false; $("#sheet").scrollTop = 0; }
function closeSheet(){ $("#backdrop").hidden = true; sheetCtx = null; ps = null; pe = null; pp = null; }
...
function bedBarHtml(bedId, opt){
  ...
  items.filter(x=>x.sp.kind!=="plant").forEach(x=>{
    const a = pc(x.sp.from), w = Math.max(0.5, pc(x.sp.to) - a);
    const cls = x.sp.precision==="slot" ? " slot" : x.sp.precision==="unknown" ? " unk" : "";
    h += `<span class="bb-band${cls}${x.planned?" plan":""}" ${opt.mini?"":`data-act="planting" data-p="${x.p.id}"`} style="left:${a}%;width:${w}%" title="${esc(x.c.name)}"><span>${esc(x.sp.precision==="unknown"?"位置不明："+x.c.name:x.c.name)}</span></span>`;
  });
  items.filter(x=>x.sp.kind==="plant").forEach(x=>{
  ...
  if(opt.cand) h += `<span class="bb-plant cand" style="left:${pc(opt.cand.x)}%">${esc(iconOf(opt.cand.crop))}</span>`;
...
function openPlace(bedId, x, anchor){
  pp = { bed:bedId, x0:x, x:x, anchor, cropId:null, date:null, add:null, variety:"", startDone:false };
  renderPlaceSheet();
}
/* 同じ栽培のまとまり（作付け）に1株追加できるか：作物・畝・開始日・品種がすべて同じ、畝の中の位置で置いた作付けだけ */
...
const plantSpot = x => ({ kind:"plant", center:x, precision:"exact" });
const placeErrors = (x, cropId, date) => checkAt([plantSpot(x)], pp.bed, cropId, date).filter(w=>w.level==="error");
/* 最初の位置：株の右（左）なら、両方の株間が分かっていればその距離、分からなければ重ならない最小の距離から探す */
function initialX(cropId){
  const L = bedDims(pp.bed).lengthCm, date = pp.date;
  const free = x => x >= 0 && x <= L && !placeErrors(x, cropId, date).length;
  const tight = x => spacingIssues(plantSpot(x), pp.bed, cropId, date).length > 0;
  if(pp.anchor){
    const a = state.plantings.find(p=>p.id===pp.anchor.pid), sp = a && spotsOf(a)[pp.anchor.i];
    if(sp){
      const base = sp.kind==="plant" ? sp.center : (pp.anchor.side==="right" ? sp.to : sp.from);
      const s1 = spacingOf(cropById(a.cropId)), s2 = spacingOf(cropById(cropId));
      const dist = sp.kind!=="plant" ? GRID : (s1 && s2) ? Math.ceil((a.cropId===cropId ? s2.min : (s1.min+s2.min)/2) / GRID) * GRID : 2*PLANT_HALF;
      const dir = pp.anchor.side==="left" ? -1 : 1;
      for(let k=0; k<200; k++){ const x = base + dir*(dist + k*GRID); if(x < 0 || x > L) break; if(free(x)) return x; }
      return snapCm(Math.max(0, Math.min(L, base + dir*dist)));
    }
  }
  if(pp.x0!=null) return pp.x0;
  let fallback = null;                                                    // 左から、重ならず株間も足りる所 → なければ重ならない所
  for(let x=0; x<=L; x+=GRID){ if(free(x)){ if(!tight(x)) return x; if(fallback==null) fallback = x; } }
...
function ppSync(){
  ...
  const di = $("#pp-date"), vi = $("#pp-variety"), ad = document.querySelector('input[name="pp-add"]:checked'), st = document.querySelector('input[name="pp-start"]:checked');
...
function renderPlaceSheet(){
  ...
  let h = "";
  const where = pp.anchor ? (()=>{ const a = state.plantings.find(p=>p.id===pp.anchor.pid); return a ? `${cropById(a.cropId).name}の${pp.anchor.side==="left"?"左":"右"}` : ""; })()
              : pp.x0!=null ? `左から${pp.x0}cm付近` : "空いている所";
  if(!pp.cropId){
    const my = (state.settings.myCrops||[]).length ? state.settings.myCrops : DATA.crops.map(c=>c.id);
    const order = {optimal:0, late:1, upcoming:2, offSeason:3};
    const rows = my.map(cropById).filter(Boolean).map(c=>({c, st:seasonStatus(c), plant:layoutOf(c)==="plant"}))
      .sort((a,b)=>(a.plant===b.plant?0:a.plant?-1:1) || order[a.st.state]-order[b.st.state]);
    h = `<h3>${esc(bed.name)}・${esc(where)}に植える</h3>
      <div class="sheet-sub">株で置く作物を選びます。すじまき・帯の作物は、今は「区画でかんたん登録」から登録してください。</div>
      <div class="croplist">${rows.map(r=>`<button class="croprow${r.plant?"":" ng"}" data-act="pp-crop" data-c="${r.c.id}"${r.plant?"":" disabled"}>
        <span class="crname">${esc(iconOf(r.c))} ${esc(r.c.name)}</span>
        <span class="crtags">${r.plant ? (spacingOf(r.c) ? `<span class="tag">株間${spacingOf(r.c).min}cm〜</span>` : `<span class="tag">株間の目安なし</span>`) : `<span class="tag">帯（次の段階）</span>`}</span>
        <span class="crwhen">${statusChip(r.st)}</span></button>`).join("")}</div>
      <div class="sheet-foot"><button class="btn" data-act="close">閉じる</button></div>`;
    showSheet(h); return;
  }
  const c = cropById(pp.cropId), sc = spacingOf(c);
  ...
  const ws = checkAt([plantSpot(pp.x)], pp.bed, pp.cropId, date);
  const old = scriptTooOld();
  /* 両隣の株 */
  const nb = barItems(pp.bed).filter(x=>x.sp.kind==="plant").map(x=>({name:x.c.name, d:x.sp.center - pp.x})).filter(x=>x.d!==0);
  const lft = nb.filter(x=>x.d<0).sort((a,b)=>b.d-a.d)[0], rgt = nb.filter(x=>x.d>0).sort((a,b)=>a.d-b.d)[0];
  h = `<button class="linkback" data-act="pp-back">← 作物を選び直す</button>
    <h3>${esc(iconOf(c))} ${esc(c.name)}を1株置く</h3>
    ...
    ${bedBarHtml(pp.bed, {mini:true, cand:{x:pp.x, crop:c}})}
    <div class="posrow">
      <button class="btn" data-act="pp-move" data-v="-${GRID}" aria-label="左へ${GRID}cm">◀</button>
      <div class="posval"><b>左から${pp.x}cm</b><span>${lft?`左の${esc(lft.name)}まで${-lft.d}cm`:"左は空き"}／${rgt?`右の${esc(rgt.name)}まで${rgt.d}cm`:"右は空き"}</span></div>
      <button class="btn" data-act="pp-move" data-v="${GRID}" aria-label="右へ${GRID}cm">▶</button>
    </div>
    ${bs.length ? `<div class="label-s">どの作付けにするか</div><div class="startchoice" role="radiogroup">
      ${bs.map(b=>`<label><input type="radio" name="pp-add" value="${b.id}"${pp.add===b.id?" checked":""}> ${esc(c.name)}（${jp(parseD(startDateOf(b)))}${b.variety?`・${esc(b.variety)}`:""}・${b.count}株）に1株追加</label>`).join("")}
      <label><input type="radio" name="pp-add" value=""${pp.add?"":" checked"}> 新しい作付けにする</label></div>` : ""}
    ${batch ? `<div class="hint" style="margin-top:6px">追加する株は、この作付けの${esc(c.start.action)}日・品種・やることをそのまま使います。</div>` : `
    <div class="row2">
    ...
    ${old ? `<div class="warn"><span class="wt">スクリプトの更新が必要です</span>畝の中に株を置くには、スプレッドシート側の Code.gs を v5 に更新してください（README「11」）。更新前に置くと、位置がシートに残りません。</div>` : ""}
    ${warnHtml(ws)}
    <div class="sheet-foot"><button class="btn" data-act="close">やめる</button>
      <button class="btn primary" data-act="pp-save"${!old && !blocking(ws) ? "" : " disabled"}>${batch?"1株追加する":"置く"}</button></div>`;
  showSheet(h);
  const re = ()=>{ ppSync(); const bs2 = batchesFor(pp.bed, pp.cropId, pp.date, pp.variety); if(!pp.add && bs2.length && !document.querySelector('input[name="pp-add"]')) pp.add = bs2[0].id; renderPlaceSheet(); };
  const di = $("#pp-date"); if(di) di.addEventListener("change", ()=>{ ppSync(); pp.startDone = parseD(pp.date) < today(); pp.add = null; const b2 = batchesFor(pp.bed, pp.cropId, pp.date, pp.variety); if(b2.length) pp.add = b2[0].id; renderPlaceSheet(); });
  const vi = $("#pp-variety"); if(vi) vi.addEventListener("change", ()=>{ ppSync(); pp.add = null; const b2 = batchesFor(pp.bed, pp.cropId, pp.date, pp.variety); if(b2.length) pp.add = b2[0].id; renderPlaceSheet(); });
  document.querySelectorAll('input[name="pp-add"]').forEach(r=>r.addEventListener("change", re));
  document.querySelectorAll('input[name="pp-start"]').forEach(r=>r.addEventListener("change", ()=>ppSync()));
}
function savePlace(){
  if(!pp || !pp.cropId || scriptTooOld()) return;
  ppSync();
  const c = cropById(pp.cropId), batch = pp.add ? state.plantings.find(p=>p.id===pp.add) : null;
  const date = batch ? startDateOf(batch) : pp.date, sp = plantSpot(pp.x);
  ...
  if(batch){
    ...
    const pid = uid("p");
    putPlanting({ id:pid, bedId:pp.bed, slots:[], spots:[sp], cropId:pp.cropId, date:pp.date, count:1, status:"active", memo:"", variety:(pp.variety||"").trim() || undefined });
  ...
  const msg = `${c.name}を${batch?"1株追加":"置き"}ました（左から${sp.center}cm）`;
...
function openSpot(pid, i){
  ...
  showSheet(`<h3>${esc(iconOf(c))} ${esc(c.name)}（${esc(spotLabel(sp))}）</h3>
...
async function handle(act, el, ev){
  ...
  switch(act){
    ...
    case "pp-move": ppSync(); pp.x = pp.x + Number(d.v); renderPlaceSheet(); break;
